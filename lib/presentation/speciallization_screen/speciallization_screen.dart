...
import 'package:rakesh_s_application2/presentation/confirmation_dialog/confirmation_dialog.dart';
...
class SpeciallizationScreen extends StatelessWidget {
  ...
  Widget _buildSpecialization(BuildContext context) {
    return Padding(
        ...
        child: BlocSelector<SpeciallizationBloc, SpeciallizationState,
                SpeciallizationModel?>(
            selector: (state) => state.speciallizationModelObj,
            builder: (context, speciallizationModelObj) {
              return ListView.separated(
                  ...
                  itemCount:
                      speciallizationModelObj?.specializationItemList.length ??
                          0,
                  itemBuilder: (context, index) {
                    SpecializationItemModel model = speciallizationModelObj
                            ?.specializationItemList[index] ??
                        SpecializationItemModel();
                    return SpecializationItemWidget(model);
                  });
            }));
  }
